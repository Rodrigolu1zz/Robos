...
public class Atlas7 extends AdvancedRobot {
    ...
    public void onScannedRobot(ScannedRobotEvent event) {
        if (missedShots <= 5) {
            ...
            // Prever a posição futura do robô oponente
            double bulletSpeed = 20 - 3 * gunPower;
            long time = (long) (distance / bulletSpeed);

            // Calcula a posição futura do robô inimigo
            double enemyHeading = event.getHeading();
            double enemyVelocity = event.getVelocity();
            double futureX = getX() + Math.sin(Math.toRadians(getHeading() + event.getBearing())) * distance + Math.sin(Math.toRadians(enemyHeading)) * enemyVelocity * time;
            double futureY = getY() + Math.cos(Math.toRadians(getHeading() + event.getBearing())) * distance + Math.cos(Math.toRadians(enemyHeading)) * enemyVelocity * time;

            // Mirar na posição futura do robô oponente
            double gunTurn = Utils.normalRelativeAngleDegrees(Math.toDegrees(Math.atan2(futureX - getX(), futureY - getY())) - getGunHeading());
